...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (stillExists) {
        ...
        UIAlertController *warning = [UIAlertController alertControllerWithTitle:@"File Removed"
                                                                         message:@"The file at the specified path no longer exists."
                                                                  preferredStyle:UIAlertControllerStyleAlert];
        UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:@"OK" style:UIAlertActionStyleCancel handler:nil];
        [warning addAction:cancelAction];
        [self.navigationController presentViewController:warning animated:YES completion:nil];
...
- (BOOL)tableView:(UITableView *)tableView shouldShowMenuForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!error ) {
        ...
        UIMenuItem *shareMenuItem = [[UIMenuItem alloc] initWithTitle:@"Share Path" action:@selector(fileBrowserPathShare:)];
        [menus addObject:shareMenuItem];

        if (fileType != NSFileTypeDirectory) {
            UIMenuItem *shareMenuItem = [[UIMenuItem alloc] initWithTitle:@"Share File" action:@selector(fileBrowserFileShare:)];
            [menus addObject:shareMenuItem];
        }
    }
...
{
    return action == @selector(fileBrowserDelete:) || action == @selector(fileBrowserRename:) || action == @selector(fileBrowserPathShare:) || @selector(fileBrowserFileShare:);
}
...
- (void)fileBrowserRename:(UITableViewCell *)sender
{
    ...
    [self.fileOperationController showInViewController:self];
}
...
- (void)fileBrowserPathShare:(UITableViewCell *)sender {
    NSIndexPath *indexPath = [self.tableView indexPathForCell:sender];
    NSString *fullPath = [self filePathAtIndexPath:indexPath];
...
- (void)fileBrowserFileShare:(UITableViewCell *)sender {
    NSIndexPath *indexPath = [self.tableView indexPathForCell:sender];
    NSString *fullPath = [self filePathAtIndexPath:indexPath];
    ...
    // Use FileURL to trigger ShareSheet.
...
- (void)fileBrowserPathShare:(UIMenuController *)sender {
    id target = [self.nextResponder targetForAction:_cmd withSender:sender];
    [[UIApplication sharedApplication] sendAction:_cmd to:target from:self forEvent:nil];
}

- (void)fileBrowserFileShare:(UIMenuController *)sender {
    id target = [self.nextResponder targetForAction:_cmd withSender:sender];
    [[UIApplication sharedApplication] sendAction:_cmd to:target from:self forEvent:nil];
}
